...
int n, k;
vector<int> a;
int dp[100005][40]; // memo table



int f(int i, int d) {
    if (i == n) return 0;
    if (d > 33) return -INF; // limit key depth
    if (dp[i][d] != -1) return dp[i][d];

    int use_good = (a[i] >> d) - k + f(i + 1, d);
    int use_bad = (a[i] >> (d + 1)) + f(i + 1, d + 1);

    return dp[i][d] = max(use_good, use_bad);
}
