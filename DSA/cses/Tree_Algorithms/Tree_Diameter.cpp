...
int _,fartthestDistancefromnode0=1, farthestnode;
vector<int> __;
vector<vector<int>> __2d,adj;

void dfs(int src , int depth , int parent ) {
      if(depth > fartthestDistancefromnode0) {
            fartthestDistancefromnode0 = depth;
            farthestnode = src;
      }
...
void solve(){
      ...
      dfs(0,1,-1);
      cerr << farthestnode<< endl;

      cout <<  dfs1(farthestnode,-1) -1  ;
