#include <bits/stdc++.h>
using namespace std;
#define int long long int
#define pb push_back
#define pll pair<int, int>
const int modll = 1e17;
const int mod = 1e9 + 7;
const int INF = 1e18;
int dx[] = {1, 0, -1, 0};
int dy[] = {0, 1, 0, -1};
vector<int> seive(int n) { vector<int> prime(n+1, 1); for(int i=2; i*i<=n; i++) if(prime[i]) for(int j=i*i; j<=n; j+=i) prime[j]=0; return prime; }
template <typename T> void takeInput(vector<T> &a, int n) { for(int i=0; i<n; i++) { T ele; cin>>ele; a[i]=ele; } }
template <typename T> void printArr(vector<T> &a) { for(auto it:a) cout<<it<<" "; cout<<endl; }
int fastPow(int a, int b) { int res=1; while(b) { if(b&1) res=(res*a)%mod; a=(a*a)%mod; b>>=1; } return res; }
int gcd(int a, int b) { if(b==0) return a; return gcd(b, a%b); }
int lcm(int a, int b) { return (a*b)/gcd(a, b); }
int modInverse(int a) { return fastPow(a, mod-2); }
int modAdd(int a, int b) { return (a + b) % mod; }
int modSub(int a, int b) { return (a - b + mod) % mod; }

int _,n,m;
vector<int> __,vis,vis2,ans;
vector<vector<int>> __2d,adj,revadj;
/**
 * 
 * 1. Sort al lthe edges according to finishing time 
 * this is done to know the 1st strongly connected component and so on.
 * 2.revverse the edges 
 * 3.do dfs
 * 
 */

void dfs(int node , stack<int>&st ) {
      vis[node] =1 ;
      for(auto it : adj[node] ) {
            if(!vis[it])
                  dfs(it,st);
      }
      st.push(node);
}

void dfs2(int node,int curr ) {
      vis2[node ]=1 ;
      ans[node] = curr;
      for(auto it : revadj[node]) {
            if(!vis2[it] )
                  dfs2(it,curr);
      }
}
void solve(){
      cin >> n >> m ;
      adj.resize(n);
      vis.resize(n);
      vis2.resize(n);
      revadj.resize(n);

      for(int i=0 ;i < m ;i ++ ) {
            int a ,b ;
            cin >> a >>b ;
            a --,b -- ;
            adj[a].push_back(b);
      }
      stack<int> st;
      // v + e 
      for(int i=0 ;i < n ; i ++ ){
            if(!vis[i]){
                  dfs(i,st);
            }
      }
      for(int i=0 ;i < n;i ++ ){
            for(auto it: adj[i]) {
                  revadj[it].push_back(i);
            }
      }
      ans.resize(n);
      //  v + v 
      int curr =1 ;

      while(!st.empty()) {
            int node = st.top();
            st.pop();
            if(!vis2[node]) {
                  dfs2(node,curr);
                  curr ++ ;
            }
      }
      cout << curr - 1 << endl;

      for(auto it : ans ) {
            cout << it << " ";
      }
      
      

     
}
int32_t main(){
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    cout.tie(NULL);
    auto start = chrono::high_resolution_clock::now();
    int t = 1;
//     cin >> t;
    while(t--){
        solve();
    }
    auto end = chrono::high_resolution_clock::now();
    auto elapsed = chrono::duration_cast<chrono::nanoseconds>(end - start);
    cerr << fixed << setprecision(10) << "Time measured: " << static_cast<double>(elapsed.count()) * 1e-9 << " seconds.\n";
    return 0;
}
