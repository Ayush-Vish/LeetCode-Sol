...
public:
    virtual bool canMove(Board* board, Piece* piece, int startX, int startY, int endX, int endY) = 0;
...
public:
    ...
    bool canMove(Board* board, int startX, int startY, int endX, int endY) {
        return movementStrategy->canMove(board, this, startX, startY, endX, endY);
    }
    virtual ~Piece() {
...
public:
    bool canMove(Board* board, Piece* piece, int startX, int startY, int endX, int endY) override {
...
class PeiceFactory {
    ...
    static Piece* createPiece(string type, bool isWhite) {
        ...
        return nullptr; // For simplicity, we only implement King and Bishop here
    }
};
...
/**
 * 
 * now Discuss about the Extensibility and maintainability of the design
 * - The use of the Strategy pattern for piece movements allows us to easily add new piece types
 * - The Singleton pattern for the Board ensures that we have a single source of truth for the game state
 */



class PlayerStreagety  {
    public:
    virtual void makeMove(Game* game) = 0;
    virtual ~PlayerStreagety() {}
};

class HumanPlayer : public PlayerStreagety {
    public:
    void makeMove(Game* game) override {
        // Implement logic to get move input from the user and make the move
    }
};

class AIPlayer : public PlayerStreagety {
    public:
    void makeMove(Game* game) override {
        // Implement logic for AI to decide on a move and make it
    }
};

class Player {
    PlayerStreagety * strategy;
    public:
    Player(PlayerStreagety* strategy) : strategy(strategy) {}
...
int main() {
    ...
    Game game(new Player(new HumanPlayer()), new Player(new AIPlayer()));
    game.start();
